...
@displays.route('/beers_tv_screen')
@login_required
def beers_tv_screen():
    user = User.query.filter_by(id=current_user.id).first()
    beers = db.session.query(
    ...
    if len(beers) > 0:
        return render_template('beers_tv_screen.html', beers=beers, beerlistFirstHalf=beerlistFirstHalf, beerlistSecondHalf=beerlistSecondHalf, beerlistBom=beerlistBom, beerlistCs=beerlistCs, currentUserId=current_user.id)
    else:
        msg = 'No Beers Found'
    return render_template('beers_tv_screen.html', msg=msg, currentUserId=current_user.id)



@displays.route('/tv_display/<string:beer_id>', methods=['GET', 'POST'])
@login_required
def tb_display(beer_id):
    user = User.query.filter_by(id=current_user.id).first()
    beers = user.beerlist_current
    beers = db.session.query(
        List_current.id,
        ...
        List_history.website,
        List_history.description,
        ).outerjoin(List_current, List_history.id == List_current.id_history).all()
    beers = db.session.query(
        List_current.id,
        List_history.id,
        List_history.name,
        ...
        # ).filter(List_current.venue_db_id == current_user.id).all()
        ).filter(List_current.venue_db_id == beer_id).all()
    beers = beers[0:22]
    beers_01_08 = beers[0:8]
    beers_08_16 = beers[8:16]
    beers_16_22 = beers[16:22]
    # return jsonify(beers)
    if len(beers) > 0:
        return render_template('beers_tv_screen.html', beers=beers, beers0108=beers_01_08, beers0816=beers_08_16, beers1622=beers_16_22, currentUserId=current_user.id)
    else:
        msg = 'No Beers Found'
    return render_template('beers_tv_screen.html', msg=msg, currentUserId=current_user.id)
